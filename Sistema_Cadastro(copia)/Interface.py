...
# Criando a variavel que vai posicionar e receber a exibição grafica do banco

# Função responsavel por exibir o dados na interface
...
# CÓDIGO DO TAMANHO DA TELA E DA FIXAÇÃO HORIZONTAL E VERTICAL
...
lb = Label(Tela, image=Logo, bg="#722B2B").place(x=280, y=10)
# Label do Logo
...
# Cria a variavel de controle e cria o campo de entrada de dados.
    def Buscar_Dados(Codigo, Janela_Relatorio):    
        ...
        if Dados:
            ...
            for i, Rotulos in enumerate(["Codigo:", "Nome:", "Endereço:","CEP:","Telefone:","Pedido:"]):
                tk.Label(Janela_Relatorio,bg="#722B2B",fg="#FFFFFF",font="bold", text=Dados[i]).grid(row=i, column=1,padx=10,pady=5, sticky="w")
...
def Exibir_Registros():     
    ...
    Janela_Dados.geometry("1300x700")
    Janela_Dados.resizable(width=False, height=False)


    VisualizarDados = tk.ttk.Treeview(Janela_Dados, columns=("Codigo", "Nome", "Endereço", "CEP", "Telefone", "Pedido"), show="headings")
...
#CODIGO DOS BOTÕES
tk.Button(Tela,text = "Cadastrar Pedido", padx=12, background="#C66B1D",fg="#FFFFFF",command=Janela_Coleta).place(x=410, y = 180)
# Botão para confirmar um pedido e salvar as informações do cliente e do pedido no banco
tk.Button(Tela,text = "Alterar Pedido", padx=20, background="#C66B1D",fg="#FFFFFF",command=Botao_Alterar_Dados).place(x=410, y = 210)
# Botão para alterar o pedido.
tk.Button(Tela,text = "Relatório do Pedido", padx=6, background="#C66B1D",fg="#FFFFFF",command=Botao_Abrir_Janela_Relatorio).place(x=410, y = 240)
# Botão parar gerar um relatório pelo código do pedido.
tk.Button(Tela,text = "Cancelar Pedido", padx=15, bg="#C66B1D",fg="#FFFFFF",command=Botao_Abrir_Janela_Exclusao).place(x=410, y = 270)
# Botão para cancelar o pedido do cliente.
tk.Button(Tela,text="Exibir Dados", padx=26, background="#C66B1D",fg="#FFFFFF",command=Exibir_Registros).place(x=410, y=300)
